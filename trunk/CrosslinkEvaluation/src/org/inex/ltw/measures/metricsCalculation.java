...
public final class metricsCalculation {
    ...
    private static Hashtable getRunSet(File runfiles) {
        ...
        try {
            ...
            JAXBContext jc;
            jc = JAXBContext.newInstance("org.inex.ltw.rungenerator");
            Unmarshaller um = jc.createUnmarshaller();
            ...
            for (int i = 0; i < is.getTopic().size(); i++) {
                ...
                if (!is.getTopic().get(i).getOutgoing().getLink().isEmpty()) {
                    ...
                    for (int j = 0; j < is.getTopic().get(i).getOutgoing().getLink().size(); j++) {
                        ...
                        String toFileID = "";
                        String toBep = "";
                        List<org.inex.ltw.rungenerator.LinktoType> linkTo = is.getTopic().get(i).getOutgoing().getLink().get(j).getLinkto();
                        for (int k = 0; k < linkTo.size(); k++) {
                            ...
                            if (!outF2FV.contains(toFileID)) {
                                ...
                                log(topicID + "<-- Topic ID: Duplicated: " + toFileID);
                            }
                        }
                    }
