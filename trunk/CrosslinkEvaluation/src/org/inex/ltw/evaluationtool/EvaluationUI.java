...
import org.inex.ltw.measures.metricsCalculation;
import org.inex.ltw.measures.plotsCalculation;
import org.inex.ltw.util.ioFileFilter;

import org.jCharts.chartData.ChartDataException;
import org.jCharts.properties.PropertyException;

import org.inex.ltw.util.JChartDialog;
import org.inex.ltw.resultsetGenerator.LtwResultsetType;
import org.inex.ltw.rungenerator.InexSubmission;
import org.inex.ltw.rungenerator.TopicType;
import org.inex.ltw.util.jfcFileFilter;
import org.inex.ltw.util.runsXMLConvertors;
...
//            JOptionPane.showMessageDialog(this, ex.toString(), "Exception Board", JOptionPane.ERROR_MESSAGE);
    ...
    private void uploadButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_uploadButtonActionPerformed
        ...
        for (int k = 0; k < fileList.size(); k++) {
            try {
                JAXBContext jc = JAXBContext.newInstance("org.inex.ltw.rungenerator");
                        ...
                        if (topic.getOutgoing().getLink().isEmpty()) {
                            ...
                            if (topic.getOutgoing().getLink().get(0).getLinkto().isEmpty()) {
                                ...
                                for (int j = 0; j < topic.getOutgoing().getLink().size(); j++) {
                                    List<org.inex.ltw.rungenerator.LinktoType> linkTo = topic.getOutgoing().getLink().get(j).getLinkto();
...
//                                result[4] = "" + topic.getOutgoing().getLink().size() + "(" + outNonduplicateCount + ")";
    ...
    private static Hashtable ResultSetLinksNo(File resultfiles) {
        ...
        try {
            JAXBContext jc;
            jc = JAXBContext.newInstance("org.inex.ltw.resultsetGenerator");
            Unmarshaller um = jc.createUnmarshaller();
